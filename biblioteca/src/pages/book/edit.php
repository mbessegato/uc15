...
if (isset($_POST["title"]) && isset($_POST["isbn"]) && isset($_POST["pages"]) && isset($_POST["language"]) && isset($_POST["edition"]) && isset($_POST["year"]) && isset($_POST["editora"]) && isset($_POST["generos"]) && isset($_POST["autores"]))
    updateBookAction($conn, $_POST["id"], $_POST["title"], $_POST["isbn"], $_POST["pages"], $_POST["language"], $_POST["edition"], $_POST["year"], $_POST["editora"], $_POST["generos"], $_POST["autores"]);
...
<div class="content">
    <h2 style="margin: 0px;">EDITAR LIVRO</h2>
    <form name="feditclient form" action="../../pages/book/edit.php" method="POST" autocomplete="on">
        <input type="hidden" name="id" value="<?= $book['id'] ?>" required />
        <label for="ftitulo" class="flabel">Título</label> <br>
        <input type="text" class="ftext" id="ftitulo" placeholder="Título do Livro" name="title" value="<?= htmlspecialchars($book['titulo']) ?>" required> <br>
        <label for="fisbn" class="flabel">ISBN</label> <br>
        <input type="number" class="ftext" id="fisbn" placeholder="1234567890123" name="isbn" value="<?= htmlspecialchars($book['isbn']) ?>" required> <br>
        <label for="fnpgs" class="flabel">Nº Páginas</label> <br>
        <input type="number" class="ftext" id="fnpgs" placeholder="188" name="pages" value="<?= htmlspecialchars($book['npgs']) ?>" required> <br>
        <label for="fidioma" class="flabel">Idioma</label> <br>
        <input type="text" class="ftext" id="fidioma" placeholder="Português" name="language" value="<?= htmlspecialchars($book['idioma']) ?>" required> <br>
        <label for="fedicao" class="flabel">Edição</label> <br>
        <input type="number" class="ftext" id="fcep" placeholder="4" name="edition" value="<?= htmlspecialchars($book['edicao']) ?>" required> <br>
        <label for="fano" class="flabel">Ano Lançamento</label> <br>
        <input type="number" class="ftext" id="fano" placeholder="1888" name="year" value="<?= htmlspecialchars($book['ano']) ?>" required> <br>
